...
description: "Notifications Verification captures the verification workflow inside notifications. It spans 2 workspaces. Key flows include Credential validation, Auth login, Auth login."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "features/notification-verify.md"
  featureId: "notification-verify"
  domain: "notification"
  action: "verify"
  fileCount: 25
---
...
Notifications Verification captures the verification workflow inside notifications. It spans 2 workspaces. Key flows include Credential validation, Auth login, Auth login.
...
flowchart LR
  ...
  actor_2["End user"]
  feature_notification_verify["Notifications Verification\nFeature boundary"]
  actor_1 --> feature_notification_verify
  actor_2 --> feature_notification_verify
  ext_1["bun"]
  feature_notification_verify --> ext_1
  ext_2["zod"]
  feature_notification_verify --> ext_2
  ext_3["hono"]
  feature_notification_verify --> ext_3
  ext_4["ioredis"]
  feature_notification_verify --> ext_4
  ext_5["google-auth-library"]
  feature_notification_verify --> ext_5
  ext_6["jose"]
  feature_notification_verify --> ext_6
```

## Detail Design

- Data stores: Session / token state, Primary database
- Integrations: bun, zod, hono, ioredis, google-auth-library, jose, node, postgres, nodemailer, minio, @, react, react-native, react-native-safe-area-context, react-native-reanimated, expo-linear-gradient, expo-status-bar, react-native-keyboard-aware-scroll-view
