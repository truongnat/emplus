---
title: "User Management Notify"
description: "User Management Notify captures the notify workflow inside user management. It spans 2 workspaces."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "features/user-notify.md"
  featureId: "user-notify"
  domain: "user"
  action: "notify"
  fileCount: 31
---

# User Management Notify
...
User Management Notify captures the notify workflow inside user management. It spans 2 workspaces.
...
flowchart LR
  ...
  feature_user_notify["User Management Notify\nFeature boundary"]
  actor_1 --> feature_user_notify
  ext_1["bun"]
  feature_user_notify --> ext_1
  ext_2["@hono"]
  feature_user_notify --> ext_2
  ext_3["hono"]
  feature_user_notify --> ext_3
  ext_4["node"]
  feature_user_notify --> ext_4
  ext_5["postgres"]
  feature_user_notify --> ext_5
  ext_6["zod"]
  feature_user_notify --> ext_6
```

## Detail Design

- Data stores: Primary database, Session / token state
- Integrations: bun, @hono, hono, node, postgres, zod, ioredis, @faker-js, google-auth-library, jose, nodemailer, minio, @, @expo-google-fonts, expo-font, expo-router, expo-splash-screen, expo-status-bar, react, react-native, react-native-safe-area-context, react-native-reanimated, expo-linear-gradient, @react-native-async-storage, expo-secure-store, react-native-keyboard-aware-scroll-view
