...
description: "Notifications Login captures the login workflow inside notifications. It spans 2 workspaces. Key flows include Auth login, Auth registration, Auth login."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "features/notification-login.md"
  featureId: "notification-login"
  domain: "notification"
  action: "login"
  fileCount: 34
---
...
- Entry points (FE): mobile/app/(tabs)/notifications.tsx, mobile/src/features/auth/components/VerifyOtpHeroSection.tsx, api/src/__tests__/auth.test.ts, api/src/dto/auth.dto.ts, api/src/dto/live.dto.ts, api/src/middleware/auth.ts, api/src/modules/admin.ts, api/src/modules/auth.ts
...
flowchart LR
  ...
  actor_2["End user"]
  feature_notification_login["Notifications Login\nFeature boundary"]
  actor_1 --> feature_notification_login
  actor_2 --> feature_notification_login
  ext_1["bun"]
  feature_notification_login --> ext_1
  ext_2["@hono"]
  feature_notification_login --> ext_2
  ext_3["hono"]
  feature_notification_login --> ext_3
  ext_4["node"]
  feature_notification_login --> ext_4
  ext_5["postgres"]
  feature_notification_login --> ext_5
  ext_6["zod"]
  feature_notification_login --> ext_6
```

## Detail Design

- Data stores: Primary database, Session / token state
- Integrations: bun, @hono, hono, node, postgres, zod, ioredis, @faker-js, google-auth-library, jose, nodemailer, minio, @, @expo-google-fonts, expo-font, expo-router, expo-splash-screen, expo-status-bar, @expo, @react-navigation, expo-blur, expo-haptics, expo-linear-gradient, react, react-native, react-native-safe-area-context, react-native-reanimated, react-native-keyboard-aware-scroll-view, @tanstack
