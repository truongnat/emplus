---
title: "Search Notify"
description: "Search Notify captures the notify workflow inside search. It spans 2 workspaces."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "features/search-notify.md"
  featureId: "search-notify"
  domain: "search"
  action: "notify"
  fileCount: 33
---
...
Search Notify captures the notify workflow inside search. It spans 2 workspaces.

### Boundaries

- Workspaces: @emplus/api, @emplus/mobile
- Entry points (FE): mobile/src/toast-context.tsx, api/src/__tests__/app.test.ts, api/src/__tests__/validation.test.ts, api/src/db/seed.ts, api/src/dto/auth.dto.ts, api/src/dto/user.dto.ts, api/src/middleware/rate-limit.ts, api/src/modules/live.ts
...
flowchart LR
  actor_1["Caller"]
  feature_search_notify["Search Notify\nFeature boundary"]
  actor_1 --> feature_search_notify
  ext_1["bun"]
  feature_search_notify --> ext_1
  ext_2["@hono"]
  feature_search_notify --> ext_2
  ext_3["hono"]
  feature_search_notify --> ext_3
  ext_4["node"]
  feature_search_notify --> ext_4
  ext_5["postgres"]
  feature_search_notify --> ext_5
  ext_6["zod"]
  feature_search_notify --> ext_6
```

## Detail Design

- Data stores: Primary database, Session / token state
- Integrations: bun, @hono, hono, node, postgres, zod, ioredis, @faker-js, google-auth-library, jose, nodemailer, minio, @, @expo-google-fonts, expo-font, expo-router, expo-splash-screen, expo-status-bar, react, react-native, react-native-safe-area-context, react-native-reanimated, expo-linear-gradient, react-native-gesture-handler, @react-native-async-storage, expo-secure-store, react-native-keyboard-aware-scroll-view, @tanstack

### Component Diagram

```mermaid
flowchart LR
  cmp_1["toast-context.tsx\nUI surface"]
  cmp_2["app.test.ts\nEntry point"]
...
| [mobile/src/presentation/hooks/auth/useForgotPasswordRequest.ts](../reference/files/mobile/src/presentation/hooks/auth/useForgotPasswordRequest.ts.md) | @emplus/mobile | Entry point | Matches the notify action heuristics for this feature. |
