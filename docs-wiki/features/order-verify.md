---
title: "Order Management Verification"
description: "Order Management Verification captures the verification workflow inside order management. It spans 2 workspaces. Key flows include Credential validation, Auth login, Auth login."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "features/order-verify.md"
  featureId: "order-verify"
  domain: "order"
  action: "verify"
  fileCount: 19
---
...
Order Management Verification captures the verification workflow inside order management. It spans 2 workspaces. Key flows include Credential validation, Auth login, Auth login.

### Boundaries

- Workspaces: @emplus/api, @emplus/mobile
- Entry points (FE): mobile/src/features/auth/components/VerifyOtpHeroSection.tsx, api/src/__tests__/validation.test.ts, api/src/dto/auth.dto.ts, api/src/dto/budget.dto.ts, api/src/dto/care.dto.ts, api/src/dto/couples.dto.ts, api/src/dto/live.dto.ts, api/src/dto/timeline.dto.ts
...
flowchart LR
  ...
  feature_order_verify["Order Management Verification\nFeature boundary"]
  actor_1 --> feature_order_verify
  actor_2 --> feature_order_verify
  ext_1["bun"]
  feature_order_verify --> ext_1
  ext_2["zod"]
  feature_order_verify --> ext_2
  ext_3["hono"]
  feature_order_verify --> ext_3
  ext_4["ioredis"]
  feature_order_verify --> ext_4
  ext_5["google-auth-library"]
  feature_order_verify --> ext_5
  ext_6["jose"]
  feature_order_verify --> ext_6
```

## Detail Design

- Data stores: Session / token state, Primary database
- Integrations: bun, zod, hono, ioredis, google-auth-library, jose, node, postgres, nodemailer, minio, @, @expo-google-fonts, expo-font, expo-router, expo-splash-screen, expo-status-bar, react, react-native, react-native-keyboard-aware-scroll-view, react-native-safe-area-context
