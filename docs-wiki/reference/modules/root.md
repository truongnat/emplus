---
title: "Module (root)"
description: "399 files and 1051 symbols under the project root."
layout: "doc"
outline:
  - 2
  - 3
...
docsWiki:
  ...
  page: "reference/modules/root.md"
  directory: ""
  fileCount: 399
  symbolCount: 1051
  workspace: ""
...
- Path: `.`
- Descendant files: 399
- Descendant symbols: 1051
- Languages: `CSS`, `JSON`, `JavaScript`, `Kotlin`, `Swift`, `TypeScript`, `YAML`
...
Root module is inferred as a authentication and access control area. The visible implementation layers are Entry point, UI surface, Utility. State is likely persisted in primary database, session / token state, cache / key-value store. The module also integrates with bun, node, @hono, hono, postgres, zod.
...
- Data stores: Primary database, Session / token state, Cache / key-value store
...
- The user or operator enters the flow through design-builder/src/App.tsx, which surfaces the login interaction. It then hands off to BuilderPage, builder-page.tsx.
- The user or operator enters the flow through design-builder/src/app/layout.tsx, which surfaces the login interaction. It then hands off to globals.css.
- The user or operator enters the flow through design-builder/src/app/page.tsx, which surfaces the login interaction.
...
- [docker-compose.yml](../files/docker-compose.yml.md) — $(doc.summary)
- [package.json](../files/package.json.md) — Contains metadata about the project and its dependencies.
- [skills-lock.json](../files/skills-lock.json.md) — Skills lock API documentation
