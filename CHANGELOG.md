...
- **fix(mobile):** Sửa lỗi typo tiêu đề (Kế đôi -> Ghép đôi) và tăng khoảng cách phía trên (top spacing) cho màn hình Pairing.
- **feat(api):** Bổ sung và chạy script seed data mẫu (`seed-custom.ts`) cho hai tài khoản `truongdq.dev@gmail.com` và `ptttrang2k@gmail.com` để ghép đôi mặc định.
